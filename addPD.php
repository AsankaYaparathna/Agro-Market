...
<body id="page-top">
                    ...
                    <table>
                      <tr>
                        <div class="form-group col-md-12">
                          <input type="text" name="PD1" class="form-control" id="PD1" placeholder="Enter new Poroduct" data-rule="minlen:4" data-msg="" />
                      ...
                      <tr>
                        <td>
                          <div class="col-md-3">
                                <input type="submit" name="updtF" value="Add" class="btn btn-outline-primary    button-big button-rouded">
                            </div>
                        </td>
                        <td>
                          <div class="col-md-5">
  ...
  if(isset($_POST['updtF']))
  {
    ...
    if(!empty($pd1))
    {
      ...
      $query1="SELECT * FROM `farmer` WHERE `UID`='$udd'";
      ...
      $vl=false;
        if($row[1]=="" && $vl==false)
        {
          $querypd1="UPDATE `farmer` SET `PD1`='$pd1' WHERE `UID`='$udd'";
          $resultpd1=mysqli_query($conn,$querypd1);
          $vl=true;
        }
        if($row[2]=="" && $vl==false && $row[1]!='$pd1')
        {
          $querypd1="UPDATE `farmer` SET `PD2`='$pd1' WHERE `UID`='$udd'";
          $resultpd1=mysqli_query($conn,$querypd1);
          $vl=true;
        }
        if($row[3]=="" && $vl==false && $row[1]!='$pd1' && $row[2]!='$pd1')
        {
          $querypd1="UPDATE `farmer` SET `PD3`='$pd1' WHERE `UID`='$udd'";
          $resultpd1=mysqli_query($conn,$querypd1);
          $vl=true;
        }
        if($row[4]=="" && $vl==false && $row[1]!='$pd1' && $row[2]!='$pd1' && $row[3]!='$pd1')
        {
          $querypd1="UPDATE `farmer` SET `PD4`='$pd1' WHERE `UID`='$udd'";
          $resultpd1=mysqli_query($conn,$querypd1);
          $vl=true;
        }
        if($resultpd1)
